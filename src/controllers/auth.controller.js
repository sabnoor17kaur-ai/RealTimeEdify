...
export const register = async (req, res) => {
    ...
    try{
        ...
        const result = await User.create({email, password: hashedPassword, username, isVerified: true});
        res.status(201).json({user: result, message: `User registered successfully`});
    }catch(error){
        res.status(500).json({message: error.message});
    }
...
export const login = async (req, res) => {
    ...
    try{
        ...
        // Auto-verify existing users who aren't verified (backward compatibility)
